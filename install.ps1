...
#   3. register ui-mineradio in cordis.patch.yml (idempotent - safe to re-run)
#
...
if ($isRemote) {
    ...
    $zipFile  = Join-Path $pluginsDir 'mineradio-plugin.zip'
    $extractDir = Join-Path $pluginsDir 'mineradio-plugin-extract'
...
if (-not (Test-Path $patchFile)) {
    ...
    # Match the real list entry only, not the "duplicate loader entry id: ui-mineradio" comment.
    if ($content -match '(?m)^\s*-\s+id:\s*ui-mineradio\s*$') {
...
Write-Host ''
Write-Host 'Done. Reload the Web UI (Mineradio is on by default; Settings -> Plugins -> Mineradio to toggle).' -ForegroundColor Green
